...
func TestMakeOffsets(T *testing.T) {
	...
	got, err := Iterate.MakeOffsets(10, len(exp))
	...
	got, err = Iterate.MakeOffsets(11, len(exp))
	is(T, fmt.Sprintf("%s", err), fmt.Sprintf("%s", nil), "no errors")
	is_deeply(T, got, exp)
	...
	got, err = Iterate.MakeOffsets(12, 3)
	is(T, fmt.Sprintf("%s", err), fmt.Sprintf("%s", nil), "no errors")
	is_deeply(T, got, exp)
	...
	got, err = Iterate.MakeOffsets(0, len(exp))
	is(T, fmt.Sprintf("%s", err), "Iterate.MakeOffsets(page_data, pages): page_data must be > 0", "got an error")
}
